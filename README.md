...
| Spalten ein/aus | Klick auf Spaltenheader |
| Bereich zuordnen | Filter-HUD → Bereiche → Ware anklicken |
| CW only Toggle | Stern-Icon ⭐ in der Icon-Leiste |
| Refresh-Intervall | Einstellbar in den Einstellungen |
...
- **HappyLooser** — HL HUD System Framework
...
| Toggle columns | Click column header |
| Assign zone | Filter HUD → Zones → click product |
| CW only toggle | Star icon ⭐ in icon bar |
| Refresh interval | Configurable in settings |
...
- **HappyLooser** — Framework HL HUD System
